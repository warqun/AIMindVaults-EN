---
type: standard
tags:
  - domain
updated: 2026-04-15
---

# CONTENTS_GLOSSARY — AIMindVault Terminology

> Related: [[CONTENTS_AI_RULES]] | [[CONTENTS_INDEX]]

| Term | Definition |
|------|------------|
| **Vault** | A folder-scoped knowledge store managed by Obsidian |
| **Hub vault** | AIMindVault — the central vault that manages scripts, standards, and templates |
| **Domain vault** | A vault that accumulates knowledge for a specific field or topic (PKM, diary, reading notes, etc.) |
| **Project vault** | A vault focused on goal delivery and task management |
| **Core Standards** | `_Standards/Core/` — operational standards shared across every vault |
| **Contents Standards** | `_Standards/Contents/` — custom per-topic rules for each vault |
| **Agent** | An AI tool that works inside the vault (Claude, Codex, Copilot, etc.) |
| **clone** | `clone_vault.ps1` — script that clones an existing vault |
| **BRAT** | Management tool for installing unofficial Obsidian plugins |
| **Juggl** | Interactive graph plugin that visualizes relationships between notes |
