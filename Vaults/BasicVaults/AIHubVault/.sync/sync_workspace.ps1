...
# ── Hub 경로 자동탐지 ──
# Hub 식별 마커: .sync/.hub_marker 파일 (AIHubVault 전용, 동기화 제외 대상)
if (-not $HubPath) {
    ...
    if ($HubFromScript -ne $normalizedVault -and (Test-Path (Join-Path $HubFromScript ".sync\.hub_marker"))) {
        ...
        # 2차: 로컬 사본 실행 — AIMindVaults 루트(Vaults/ 보유 조상)에서 .sync/.hub_marker 재귀 탐색
        $rootDir = ""
        $walkDir = $VaultRoot
        for ($i = 0; $i -lt 10; $i++) {
...
$SYNC_SCRIPT_VERSION = "202604060003"
...
Sync-SyncFolder -SourceSyncFolder $hubSyncFolder -TargetSyncFolder $targetSyncFolder -ExcludeFiles @(".hub_marker")
