...
|---------|------|--------|----|----|
| R064 | 2026-04-18 | **Phase 8 MCP/Plugins optimization — maintainer-local environment changes**. Global `~/.claude/settings.json` trimmed to single MCP (`notion`). Plugins `bkit` and `github` disabled globally. Domain MCPs moved to per-project `.claude/settings.json` (AIMindVaults: blender + youtube-transcript; Unity project: mcp-unity). Removed: `playwright`, `context7`, `github` (Failed). `bkit` re-enabled at the AIMindVaults project level for game-toolkit and project vault work. Backup saved at `~/.claude/settings.json.backup-20260418`. Expected reduction: MCP tools 67.3k → ~15k, Custom agents 6.3k → ~0, total baseline 170k → ~80k. Not a distribution change (both KO/EN = `–`). | – | – |
